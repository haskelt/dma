import TaskHierarchyObject from '{{ SITE_PATH }}/js/task-sequence/TaskHierarchyObject.js';
...
/*****************************************************************************/
	...
	super();
	this.task_wrappers = [];
	for (let taskWrapper of sequenceElement.querySelectorAll('.task-sequence__task-wrapper')){
	    let task_wrapper_data = {};
	    task_wrapper_data['container'] = taskWrapper.querySelector('.task-sequence__task-wrapper--container');
	    task_wrapper_data['previous_button'] = taskWrapper.querySelector('.task-sequence__task-wrapper--button[data-action="previous"]');
	    task_wrapper_data['previous_button'].addEventListener('click', this.goToPrevious.bind(this));
	    task_wrapper_data['next_button'] = taskWrapper.querySelector('.task-sequence__task-wrapper--button[data-action="next"]');
