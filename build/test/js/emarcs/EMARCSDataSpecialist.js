...
import DataSets from '../data/DataSets.js?v=0.24.0-beta';
import XLSXManager from '../xlsx/XLSXManager.js?v=0.24.0-beta';
import DataSpecialist from '../data/DataSpecialist.js?v=0.24.0-beta';
...
class EMARCSDataSpecialist extends DataSpecialist {
	...
	for(let sheet of Object.values(this.curData.Sheets)){
		...
		let top = XLSXManager.encodeAddress({c: col, r: 0});
		let bottom = XLSXManager.encodeAddress({c: col, r: 2});
	...
	for(let sheet in this.curData.Sheets){
		...
		let sheetRange = XLSXManager.decodeRange(this.curData.Sheets[sheet]['!ref']);
		for(let col = sheetRange.s.c; col <= sheetRange.e.c; col++){
		    let top = XLSXManager.encodeAddress({c: col, r: 0});
		    let bottom = XLSXManager.encodeAddress({c: col, r: 1});
	...
	for(let sheet of Object.values(this.curData.Sheets)){
	    ...
	    for(let col = sheetRange.s.c; col <= sheetRange.e.c; col++){
		let top = XLSXManager.encodeAddress({c: col, r: 0});
		let middle = XLSXManager.encodeAddress({c: col, r: 1});
		let bottom = XLSXManager.encodeAddress({c: col, r: 2});
	...
	for(let sheet of Object.values(this.curData.Sheets)){
		...
		/* Gradebook files have a header row, followed by a blank
		   ...
		   2nd row (index 1). */
		let upper_heading = XLSXManager.encodeAddress({c: col, r: 0});
		let lower_heading = XLSXManager.encodeAddress({c: col, r: 1});
		...
		let average_row = XLSXManager.encodeAddress({c: col, r: sheetRange.e.r});
	...
	let sheetRange = XLSXManager.decodeRange(firstSheet['!ref']);
	...
	for(let col = sheetRange.s.c; col <= sheetRange.e.c; col++){
	    let row2cell = XLSXManager.encodeAddress({c: col, r: 1});
	...
	var averagesCell = XLSXManager.encodeAddress({c: 0, r: sheetRange.e.r});
	...
	var pointsPossibleCell = XLSXManager.encodeAddress({c: 0, r: 2});
	...
	var offsetNameCell = XLSXManager.encodeAddress({c: 0, r: 1});
